...
window.initJournalCoach = function (core) {
  ...
  function sync() {
    ...
    $('#shutterLabel').textContent=session.phase==='recording'?txt('结束录制','Stop recording'):session.phase==='countdown'?txt('取消倒计时','Cancel'):session.phase==='finishing'?txt('正在保存…','Saving…'):cam.running?txt('开始录制','Record'):txt('开启相机','Open camera');
  ...
  session.begin=async function(...args){if(!validQuestions())return;revision++;if(audio.ctx?.state==='suspended')audio.ctx.resume().catch(()=>{});const promise=originalBegin.apply(this,args);sync();try{return await promise;}finally{sync();}};
  ...
  detail.render=function(entry,siblings){
    ...
    section.innerHTML=`<summary>${txt('文字日记与反馈','Written reflection & feedback')}</summary><div class="feedback-body"><p>${txt('只分析你确认发送的文字，不观看或评价视频里的表情、动作。','Only the text you choose to send is analyzed. Video, facial expressions and movements are not analyzed.')}</p><label for="feedbackDraft">${qaText?txt('检查识别文字，修改后再发送','Review and edit the transcript before sending'):txt('这段视频没有识别文字。写几句复盘，也可以只保存在本地。','There is no transcript. Write a reflection; you can keep it only on this device.')}</label><textarea id="feedbackDraft" rows="7" maxlength="12000" placeholder="${txt('我刚才说了什么？回看时注意到什么？下一次想试试什么？','What did I say? What did I notice when watching? What will I try next?')}">${esc(draft)}</textarea><div class="feedback-access access-wrap" ${api.requiresAccessCode?'':'hidden'}><label for="feedbackAccess">${txt('AI 访问码','AI access code')}</label><input id="feedbackAccess" type="password" autocomplete="off" value="${esc(accessCode)}"><button class="text-link" type="button" id="showFeedbackAccess">${txt('显示','Show')}</button></div><p class="coach-note">${txt('点“生成反馈”后，以上文字、主题和记录时的目标会发送给 DeepSeek。','Generate feedback sends this text, topic and the goal for this recording to DeepSeek.')}</p><div class="ai-actions"><button class="btn" id="saveFeedbackDraft">${txt('保存文字日记','Save written reflection')}</button><button class="btn solid" id="generateFeedback">${txt('生成反馈','Generate feedback')}</button></div><p class="feedback-status" role="status" aria-live="polite"></p><div class="feedback-output">${feedbackHTML(entry.coachFeedback)}</div></div>`;
    this.el.insertBefore(section,$('.actions',this.el));
