(function () {
  ...
  const DRAFT_KEY = 'zwdc:draft:v1';
  const CODE_KEY = 'zwdc:code';
  const POS_KEY = 'zwdc:pos';
  ...
  // ---------- state ----------
  const UPDATED_KEY = 'zwdc:draft:updatedAt';
  ...
  function buildMarkdown() {
    ...
    out.push('');
    out.push('```');
    out.push('以上是我做的一份自我调查，目的是想清楚职业发展和个人成长。请你：');
  ...
  function download(md) {
    ...
    const a = el('a', { href: URL.createObjectURL(blob), download: `zwdc-${stamp}.md` });
    ...
    try {
      ...
      for (const it of r.items) {
        const item = el('div', { class: 'h-item' },
          ...
          el('div', { class: 'h-btns' },
            el('a', { class: 'btn small', href: `/api/submissions/${it.id}.md?code=${encodeURIComponent(code())}`, download: `zwdc-${it.id}.md`, text: '下载 .md' }),
